import {themes as prismThemes} from 'prism-react-renderer';
import type {Config} from '@docusaurus/types';
import type * as Preset from '@docusaurus/preset-classic';

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

const config: Config = {
  title: 'TrackyFi',
  tagline: 'Personal finance tracker for India',
  favicon: 'img/favicon.png',

  // Future flags, see https://docusaurus.io/docs/api/docusaurus-config#future
  future: {
    v4: true, // Improve compatibility with the upcoming Docusaurus v4
  },

  url: 'https://gokulnathp.github.io',
  baseUrl: '/trackyfi-docs/',

  organizationName: 'gokulnathp',
  projectName: 'trackyfi-docs',
  trailingSlash: false,

  onBrokenLinks: 'throw',

  headTags: [
    {
      tagName: 'meta',
      attributes: {
        name: 'google-site-verification',
        content: 'dOIPFBjwYFaTDLhU9SiFTo1RAcPAG7SHxa0eSuRZDa8',
      },
    },
  ],

  // Even if you don't use internationalization, you can use this field to set
  // useful metadata like html lang. For example, if your site is Chinese, you
  // may want to replace "en" with "zh-Hans".
  i18n: {
    defaultLocale: 'en',
    locales: ['en'],
  },

  presets: [
    [
      'classic',
      {
        docs: false,
        blog: false,
        theme: {
          customCss: './src/css/custom.css',
        },
      } satisfies Preset.Options,
    ],
  ],

  themeConfig: {
    image: 'img/og-image.png',
    colorMode: {
      disableSwitch: true,
      defaultMode: 'dark',
    },
    navbar: {
      title: 'TrackyFi',
      logo: {
        alt: 'TrackyFi Logo',
        src: 'img/trackyfi-icon.png',
      },
      items: [],
    },
    footer: {
      style: 'dark',
      links: [
        {
          title: 'App',
          items: [
            {
              label: 'Privacy Policy',
              to: '/privacy',
            },
          ],
        },
        {
          title: 'Contact',
          items: [
            {
              label: '4gokul2000@gmail.com',
              href: 'mailto:4gokul2000@gmail.com',
            },
          ],
        },
      ],
      copyright: `Copyright © ${new Date().getFullYear()} GokulnathP.`,
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,
};

export default config;
